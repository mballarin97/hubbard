...
if not os.path.isdir(save_path):
    os.mkdir(save_path)
results_1 = hbb.postprocess_data(22)

spin = results_1["spin"]
charge = results_1["charge"]
...
distance_charge = np.vstack( (
    charge[:, 0],
    np.mean( np.hstack( (charge[:, 1].reshape(-1, 1), charge[:, 4].reshape(-1, 1)) ), axis=1),
    charge[:, 5],
    charge[:, 2],
    charge[:, 6],
    charge[:, 3],
...
figname = os.path.join(save_path, ("charge_excitation.pdf") )
with qplotter as qplt:

    im0 = qplt.ax[0].imshow(distance_charge, aspect='auto', interpolation='none', cmap="jet")
    cbar = plt.colorbar(im0, ax=qplt.ax[0])
    cbar.ax.set_ylabel('Charge $N_i(t)$', rotation=270, labelpad=15)

    qplt.ax[0].tick_params(axis='y', which='minor', left=False, right=False)
    qplt.ax[0].set_yticks( [ii for ii in range(7)])
    ...
    qplt.ax[1].set_yticks( [ii for ii in range(7)])
...
spin_correlator = results_1["spin_correlator"]
matter_correlator = results_1["matter_correlator"]
...
figname = os.path.join(save_path, ("charge_excitation_correlators.pdf") )
with qplotter as qplt:
